...
def extrapolate_biology(ds_in: xr.Dataset, ds_out: xr.Dataset) -> xr.Dataset:
    ...
    """
   #ds_in=ds_in.chunk(chunks={"time": 1})
   #ds_out=ds_out.chunk(chunks={"time": 1})
...
def merge_and_save_datasets(station: {}) -> {}:
    ...
    for ds, netcdf_file in zip([ds_temp_salt, ds_phyc_extrapolated], [netcdf_file1, netcdf_file2]):
        if os.path.exists(netcdf_file):
            os.remove(netcdf_file)
        ds.to_netcdf(netcdf_file)
        print(f"Saved dataset to file: {netcdf_file}")
    return station
...
def get_index_closest_points(
    ...
    distances, indices = tree.query(np.deg2rad(np.c_[st_lat, st_lon]), k=9)
